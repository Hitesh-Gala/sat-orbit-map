...
**Static-site caveat:** there is no backend, so by default each submission is stored in the *visitor's own* `localStorage` (`nazar.feedback.v1`) — the review panel on a given device only shows feedback left on that device. Central collection requires setting `CONFIG.endpoint` in `feedback.js` to a backend URL; when set, every submission is also forwarded there (fire-and-forget `no-cors` POST of `{action:'add', record}`). `feedback-backend.gs` is a ready Google Apps Script that **auto-emails every new comment** (contact/IP/geo/device) to the owner from their own Gmail via `MailApp.sendEmail` (and optionally logs to a private Sheet) — so the owner never has to monitor the site. The owner must deploy it under their own Google account (web app, "execute as me", "anyone" access) and paste the `/exec` URL into `CONFIG.endpoint`; I can't deploy it or send mail for them. `CONFIG.adminPassword` gates the in-page review (client-side only — treat as a convenience lock, not real security).
