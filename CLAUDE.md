...
| `debris.html` | `debris.js` | globe.gl + **bare three.js r157** | Debris Tracker — InstancedMesh globe of the four major breakup clouds (Fengyun-1C, Cosmos 2251, Iridium 33, Cosmos 1408) **plus Yaogan-50 (02)**, coloured by event, hover tooltips, per-event filter. Fetches CelesTrak per-event debris GROUPs (live → `argos.debris.tle.v1` 6 h cache → bundled `data/debris.tle`), and always merges `data/event-debris.tle` on top. **CelesTrak publishes debris GROUPs only for the four historic clouds, so a recent break-up is invisible to this page unless it is added there:** `scripts/gen_event_debris.py` lifts the fragments out of the daily Space-Track non-payload bundle (`data/spacetrack-other.json`) by launch designator into `data/event-debris.tle` (run from refresh-spacetrack.yml; keeps the old file if the pull came back empty). To add the next event: one entry in that script's `EVENTS`, one in `SOURCES` + `INFO` in `debris.js` (keyed by the same 5-char designator), and one in `EVENTS` in `gen_debris_history.py`. Fragments carry Alpha-5 catalogue numbers (`A0564` = 100564). Each fragment tagged to its source by the parent launch designator (line-1 cols 10–14). "📊 Statistics & history" links to debris-stats.html. A right-edge button opens a self-contained **2007 Chinese ASAT** long-read (story + fact sheet + public-domain photos under `data/asat/` + original inline-SVG charts) |
