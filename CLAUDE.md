...
| `cone-waffles.html` | `cone-waffles.js` | globe.gl + **bare three.js r157** | Game of Cones with a **steerable land cone** (Space Stuff card). The cone, a dashed white axis `THREE.Line` (15 % past the cone) and a grab handle sit in one group oriented from `tilt` (from local vertical) + `dir` (compass bearing of the lean); the scene-space East-North-Up frame is taken from `globe.getCoords` finite differences. Dragging the axis — hit-tested in screen pixels and captured on `#globe` before OrbitControls — re-aims it where the pointer ray meets a sphere around the apex. The left panel shows the lean on an SVG clock face (12 = N, radius = tilt, outer ring = horizon) plus the tilt in degrees, with click-to-set. The in-cone test runs in ECEF with an ENU frame and also requires the satellite to be above the local horizon; hits list range, elevation and off-axis angle. **Sat Cone** points the sensor off nadir with two ±45° sliders: along track (`fwd` = orbital direction from two satellite.js positions 10 s apart under one GMST, so it's defined even for GEO) and cross track (`right` = heading + 90°, taken geographically). Boresight = normalize(nadir + tan(along)·fwd + tan(cross)·right), marked by a non-interactive dotted line to the ground plus a dot; the footprint is ray-cast round the cone edge, with misses clamped to the horizon by bisection toward nadir. A right-hand **date & time** panel (also a `.goc-shell`, so the phone drawer takes it) runs the page live or frozen at a chosen moment up to 15 days ahead (slider, UTC/IST date + time inputs, back-to-now button; a chosen moment that slips into the past reverts to live). Its **1 hour lead** button opens a modal: every satellite inside the Land cone at some instant in [T, T+1 h] (found by 1-min Earth-fixed samples; a minute is refined second-by-second only if its midpoint lies within half the step + 60 km of the cone — a conservative distance bound — verified identical to a 5-s brute force), plotted at its T position on an SVG azimuthal-equidistant map centred on the site (sqrt-scaled ground distance, `data/countries-110m.geojson` outlines, cone reach = convex hull of projected cone rings, dashed per-5°-sector "1-hour boundary"), with hover ground track / details and a synced table; the scan yields every ~30 ms and is cancelled by a token on close. Land Cone propagates the catalogue with raw SGP4 and does the costly geodetic conversion only for satellites passing a 60 km Earth-fixed pre-check (results identical to the exhaustive test); Sat Cone propagates just its satellite |
...
| `debris.html` | `debris.js` | globe.gl + **bare three.js r157** | Debris Tracker — InstancedMesh globe (`MAX_INSTANCES` 20 k) of **every catalogued fragment**, ~12.3 k, coloured by breakup event: Fengyun-1C, Cosmos 2251, Iridium 33, Cosmos 1408, **Yaogan-50 (02)**, and "Other" (~9.1 k). Two catalogues are merged: `data/spacetrack-debris.tle` (primary; all Space-Track `OBJECT_TYPE=DEBRIS`, rebuilt daily from `data/spacetrack-other.json` by `scripts/gen_spacetrack_debris.py` in refresh-spacetrack.yml, with a 5 k floor so a thin pull can't wipe it) and CelesTrak's per-event GROUPs (live → `argos.debris.tle.v1` 6 h cache → bundled `data/debris.tle`). **CelesTrak publishes debris GROUPs for the four historic clouds only — nothing from any recent break-up — which is why Space-Track is the primary here.** `mergeCatalogues` matches on NORAD number and **Space-Track wins every conflict**; CelesTrak-only objects are still kept (usually the intact parent satellites, which CelesTrak ships inside its debris groups). Every disagreement is recorded in `purity` and rendered by the **🧪 Source purification** button (below Statistics & history): which source was staler (>12 h), element mismatches where the epochs are within an hour (orbits move — a day-apart epoch is a different measurement, not a contradiction), name mismatches, coverage gaps, and element sets SGP4 rejects. Fragments carry Alpha-5 catalogue numbers (`A0564` = 100564). **Pop-up house style on this page:** type runs ~10 % larger than the rest of the site and the overlays are *not* dimmed or blurred — the globe stays live behind them (`.deb-modal`, `.asat-modal`, and the gate card via `body.page-debris #nz-gate.nz-over`); the info card is 780 px wide so the longest panel (Yaogan-50) fits a 900 px screen without scrolling at that size. Text drawn inside the ASAT inline-SVG charts is deliberately left unscaled — a bigger face collides with the plotted labels. To add the next event to the legend: one entry in `SOURCES` + `INFO` in `debris.js` keyed by the launch designator (line-1 cols 10–14), and one in `EVENTS` in `gen_debris_history.py`. Each fragment tagged to its source by the parent launch designator (line-1 cols 10–14). "📊 Statistics & history" links to debris-stats.html. A right-edge button opens a self-contained **2007 Chinese ASAT** long-read (story + fact sheet + public-domain photos under `data/asat/` + original inline-SVG charts) |
...
**Scrollbars:** CSS promotes the *other* axis to `auto` as soon as one axis is non-visible, so a `overflow-y: auto` panel grows a **horizontal** scrollbar the moment any child is a pixel too wide (a focus ring, a letter-spaced label). Every fixed-width panel, HUD card, dropdown and modal card therefore pins `overflow-x: hidden` alongside its `overflow-y: auto` — keep doing that for new ones. Genuinely wide content (Sat-Steroids' `.tbl-wrap`, compendium's reference tables) keeps its horizontal scroller on purpose. Modal cards size to `92vh` rather than a hard pixel cap: `min(90vh, 780px)` bought the Yaogan-50 panel a scrollbar for 14 px of overflow on a 900 px screen.
...
## Password gates (`nazar-gate.js`)

One owner password protects **Indi-Space**, the whole **Space Stuff** section and **Site Analytics**; public files hold only its SHA-256 — in `nazar-gate.js` (`SHA256`), `site-analytics.js` (`CFG.passwordSha256`) and the deployed Apps Script (`PASSWORD_SHA256` in `site-analytics-backend.gs`, which needs a new deployment version). Change all three together, never commit the plaintext. Every Space Stuff page (`space-stuff`, `space-launched`, `traffic-conjunctions`, `data-comparison`, `mirror-globe`, `sat-steroids`, `cone-waffles`) loads `<script src="nazar-gate.js" data-section="spacestuff">` right after `<title>`: it hides every `<body>` child (`html.nz-locked`) behind a password card until the password is entered once per tab (`sessionStorage['nazar.spacestuff']`). New Space Stuff pages must include it. `NazarGate.ask({icon, title, sub, key})` puts the same card in front of a **single control** and resolves true/false — used by the Debris Tracker's 🧪 Source purification button (`sessionStorage['nazar.purity']`, one unlock per tab); the page itself stays open. It is a client-side gate on a static site — the data files stay publicly fetchable.
...
## Site analytics (`site-analytics.js` on every page → `site-analytics.html` dashboard)

A self-hosted visitor log backed by a **Google Sheet + Apps Script** in the owner's Google account (`site-analytics-backend.gs` — setup steps at its top; the deployed web-app `/exec` URL goes in `site-analytics.js` `CFG.endpoint`). Until the endpoint is set, logging is a no-op and the dashboard shows setup instructions.

- **Logger** (`site-analytics.js`, every page): one Sheet row per visit — session id in `sessionStorage`, a random visitor id in `localStorage` (unique-user count). `{type:'visit'}` beacons (`sendBeacon` with a `text/plain` body, so no CORS preflight) carry pages seen and **foreground seconds summed across pages**, sent on `visibilitychange→hidden` / `pagehide` plus a 5-min heartbeat; the first located beacon also carries the details — IP, country/region/city/ISP (ipwho.is → geojs fallback), device name/type, OS and browser (UA parse refined by UA client hints: real model, Windows 11, Brave…), screen, language, time zone, referrer, landing page. Skipped on the owner's machine (`localStorage['nazar.owner']`, set on unlock), for `navigator.webdriver`, and on the dashboard page.
- **Backend** (`site-analytics-backend.gs`): `doPost` upserts by session id under a script lock — beacons can arrive in any order, so it keeps the max seconds/pages, fills still-blank details, and prefixes formula-like visitor text with `'`. `{type:'read', pw}` returns the newest 5000 rows + totals only when `sha256(pw)` matches `PASSWORD_SHA256`.
- **Gate + dashboard**: About's "📊 Site Analytics" button checks the password against `CFG.passwordSha256` (the plaintext is in no public file), hands it over in `sessionStorage` and opens `site-analytics.html` (`site-analytics-view.js`), which re-verifies with the backend and shows total users to date, visitors today, visits, average time and countries, plus a filterable per-visit table (time of access, time spent, IP, device, OS, browser, country/state/city, pages, referrer, ISP, screen, language, time zone, new/returning).
- About's "Under the hood" section carries the matching visitor privacy note — keep it in sync if the logged fields change.
...
`window.NazarNews` is the shared feed layer behind both the ticker and the archive page. It pulls 12 feeds (space press, NASA/ESA, dedicated China tag feeds, plus two Google News topic searches), merges them into a **persistent, deduped localStorage archive** (`nazar.news.archive.v2`, keyed off normalised article URL, kept from 01 Jul 2026, capped 500), and never lets a partial pull wipe prior headlines. `getTickerItems()` returns Chinese orbital events first, then the rest of the China feed, then world, ≤20, last ~31 days; the archive page shows everything.

- **Why the topic searches:** every RSS feed exposes only its newest ~25 items, and the archive only holds what a visitor's browser happened to pull, so a story goes unrecorded unless someone opens the site within a day or two of publication — that is how the **Yaogan-50 (02) breakup (04 Sep 2026)** never reached the ticker although SpaceNews ran it. The two `news.google.com/rss/search?q=…when:30d` feeds (`google: true`) look 30 days back and reach outlets NAZAR doesn't subscribe to. Their titles read "Headline - Publisher" and carry `<source>`, so `splitGoogleTitle` restores the real publisher; `mergeIntoArchive` also dedupes on `titleKey` and swaps a `news.google.com` redirect for the publisher's own link when both feeds carry the story.
- `EVENT_RE` (breakup / collision / debris / failure / reentry, **title only** — descriptions mention debris in passing) promotes Chinese event items to the head of the ticker. Items with fewer than three words in the title are dropped: agency feeds mix plain site pages (NASA's "Travel" page) in with news.

- **CORS:** feeds are fetched through a proxy chain — `api.allorigins.win` (raw XML → DOMParser, primary) → `api.rss2json.com` (clean JSON, ~10 items) → `api.codetabs.com` (raw XML). First one yielding items wins. **`corsproxy.io` was the primary until Sep 2026, when it started returning HTTP 401 "a valid API key is required" for every request** — it was removed rather than left to burn a round-trip per feed. Expect ~10–13 of the 16 feeds to answer any single pull; the archive is cumulative, so that is fine.
- **Sources pop-up:** the archive page's `📡 Sources` button lists every monitored feed, built from `NN.FEEDS` so it can't drift, plus a hand-maintained `WATCHED` list (Andrew Jones/@AJ_FI, Jonathan McDowell, Byron Wan, Space-Track) — X accounts have no browser-readable feed, so they are documented, not polled. Every item must carry a real `http(s)` link or it's dropped (this is what fixed the old "link won't open" problem).
