...
| `2d-views.html` | `2d-views.js` | SVG overlay on a Blue Marble raster | Single-satellite ground-track tool. Track is a self-drawn SVG overlaid on the equirectangular basemap (lon/lat → viewBox linearly). **Time-based** mode: ±24 h SGP4 sub-point path as dotted past/future lines + 30-min direction arrows + hover place-names. **Rev-based** mode (panel toggle): dots hidden, a floating speed slider (1×–100×) flies the sat dot forward from now, tracing a golden trail capped at 3 revs, with projected UTC/IST clocks. amCharts is loaded for the worldLow GeoJSON only (point-in-polygon reverse-geocode, coarse ocean-basin fallback) |
